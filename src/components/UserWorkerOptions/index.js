...
import { WorkerCardsContainer, OptionCardLink, WorkerCard, WorkerImage, WorkerCardDesc, WorkerTitle, NextPageBtn } from "./styledComponents";
...
class UserWorkerOptions extends Component {
    ...
    getWorkerData = async () => {
        ...
        if (response.ok) {
            ...
            if (data.length !== 0) {
                this.setState({ workerData: data, apiStatus: apiStatusConstants.success })
            }
            else {
        ...
        return (
            <WorkerCardsContainer>
                {workerData.map(eachWorker => {
                    ...
                    return (
                        <OptionCardLink key={eachWorker.id} to={`/user/services/${reqType}/${eachWorker.id}`} >
                            <WorkerCard>
